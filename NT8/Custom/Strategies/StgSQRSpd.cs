...
//				giChartTrader = GIChartTrader(this.Input);
				...
				NumStdDevUp, NumStdDevDown, PTStdDev, SLStdDev, BasePriceType, 1);
...
//				adx1 = ADX(BarsArray[1], 14);
			if(giSQRSpd.IsLastBarOnChart(BipIWMSt) > 0 && BarsInProgress == BipIWMSt) {
				int spy_Ln = GetTradeQuantity(BipSpyLn, -1);
				int spy_St = GetTradeQuantity(BipSpySt, -1);
				int qqq_Ln = GetTradeQuantity(BipQQQLn, -1);
				int qqq_St = GetTradeQuantity(BipQQQSt, -1);
				int iwm_Ln = GetTradeQuantity(BipIWMLn, -1);
				int iwm_St = GetTradeQuantity(BipIWMSt, -1);
				Print(String.Format("{0}:GetQuantity Bip={1}: spy_Ln={2}, spy_St={3}, qqq_Ln={4}, qqq_St={5}, iwm_Ln={6}, iwm_St={7}",
				CurrentBar, BarsInProgress, spy_Ln, spy_St, qqq_Ln, qqq_St, iwm_Ln, iwm_St
				));
...
//				q_Ln, q_St, q_midLn, q_midSt));
		...
		public override int GetTradeQuantity(int idx, double ratio) {
			double prcBase = GetBaseSymbolPrice();
			double rtBase = GetBaseSymbolRatio();
			double prc = 0, rt = 0;
			int qnt = 0;
			switch(idx) {
				case BipSpyLn:
					prc = giSQRSpd.BaseLnSpy;
					rt = SpyLnSymbolRatio;
					break;
				case BipSpySt:
					prc = giSQRSpd.BaseStSpy;
					rt = SpyStSymbolRatio;
					break;
				case BipQQQLn:
					prc = giSQRSpd.BaseLnQQQ;
					rt = QQQLnSymbolRatio;
					break;
				case BipQQQSt:
					prc = giSQRSpd.BaseStQQQ;
					rt = QQQStSymbolRatio;
					break;
				case BipIWMLn:
					prc = giSQRSpd.BaseLnIWM;
					rt = IWMLnSymbolRatio;
					break;
				case BipIWMSt:
					prc = giSQRSpd.BaseStIWM;
					rt = IWMStSymbolRatio;
					break;
			}
			if(prcBase > 0 && prc > 0 && rt > 0)
				qnt = (int)(DefaultQuantity*prcBase*rtBase/(prc*rt));
...
//				CurrentBar, BarsInProgress, openBase, open, qnt, DefaultQuantity, rt));
		...
		private double GetBaseSymbolPrice() {
		...
		private double GetBaseSymbolRatio() {
			switch(TradeBaseSymbol) {
				case BipSpyLn:
					return SpyLnSymbolRatio;
				case BipSpySt:
					return SpyStSymbolRatio;
				case BipQQQLn:
					return QQQLnSymbolRatio;
				case BipQQQSt:
					return QQQStSymbolRatio;
				case BipIWMLn:
					return IWMLnSymbolRatio;
				case BipIWMSt:
					return IWMStSymbolRatio;
				default:
					return -1;
			}
		}
